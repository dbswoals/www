...
$(document).ready(function() {
    ...
    $('.gallery .link1 span').delay(1500).animate({top:600, opacity:1},'slow');
    $('.gallery .link1 p').delay(1500).animate({top:700, opacity:1},'slow');
    ...
    //   }
      ...
      $('.mbutton').css('width','80'); // 버튼 원래의 너비
      //자신만 불켜
      $('.btn'+cnt).css('width','200');  
      $('.mbutton').removeClass('on');
      $('.btn'+cnt).addClass('on');

      $('.gallery li span').css('top',0).css('opacity',0);
      $('.gallery li p').css('top',-1000).css('opacity',0);
      $('.gallery .link'+cnt).find('span').delay(1000).animate({top:1700, opacity:1},'slow');
   ...
   $('.mbutton').click(function(event){  //각각의 버튼 클릭시
      ...
      if(onoff==false){ //중지상태냐??
            onoff=true; //동작~~
            $('.ps').html('<span class="hidden">play</span><i class="stop sfsdfsdf"></i>');
      }
  ...
  $('.ps').click(function(){ 
     if(onoff==true){ // 타이머가 동작 중이냐??
	       clearInterval(timeonoff);   //살인마 고용 stop버튼 클릭시
		     $(this).html('<span class="hidden">stop</span><i class="stop sfsdfsdf"></i>'); //js파일에서는 경로의 기준이 html파일이 기준!!
         onoff=false;   
	   }else{  // false 타이머가 중지 상태냐??
		   timeonoff= setInterval( moveg , 4000); //play버튼 클릭시  부활
		   $(this).html('<span class="hidden">play</span><i class="play sfsdfsdf"></i>');
		   onoff=true; 
	   }
  });	
